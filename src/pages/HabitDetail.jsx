...
import { updateHabit,deleteHabit,getHabitById } from '../mockAPI'; // If moved to src/
...
export default function HabitDetails() {
  ...
  useEffect(() => {
    const fetchHabit = async () => {
      try {
        ...
        console.error('Error fetching habit:', error);
        // Optionally navigate to dashboard or show error message
        // navigate('/dashboard');
      } finally {
        setIsLoading(false);
      }
  ...
  const handleProgressUpdate = async (progress) => {
    try {
      // Send the updated progress to the mock API
      const updatedHabit = await updateHabit(id, { newProgress: progress });
  
      // Update local state to reflect changes
      setHabit(updatedHabit);
      setProgressLog(updatedHabit.progressLog);
  
      // Save the most recent progress (optional localStorage handling if needed)
      const lastProgress = updatedHabit.progressLog[updatedHabit.progressLog.length - 1].progress;
      setLastProgressUpdate(lastProgress);
      localStorage.setItem(`lastProgressUpdate-${id}`, JSON.stringify(lastProgress));
